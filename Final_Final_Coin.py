...
if __name__ == "__main__":
    ...
    # API 받아오기
    key1 = "t0PI0Qx8q77h53kJlgo8OV3T317oUpsTHBeg0uG4"
    key2 = "xSKQDDo0MhGRYbaLxmn9mz1NGqEnmM4px83QZXgU"
    ...
    try:
        ...
        while True:
            try:
                ...
                for i in range(5):  # range('코인 개수') 바꿔주기!
                    if i == 0:  # 비트코인 매매
                        ...
                        if mid09 <= now and now <= mid09 + delta:
                            ...
                            BSV_price = pyupbit.get_current_price(
                                "KRW-BSV")  # 비트코인에스브이의 현재가격
                            if BSV_target_price <= BSV_price and BSV_price >= BSV_ma5 and BSV_hold_flag == False:
                                ret = buy_crypto_currency(
                                    upbit, percent=0.33, ticker="BSV")
                                print("BSV 매수", ret)
                                BSV_hold_flag = 'already_buy'

                    elif i == 4:  # 라이트코인 매매
                        if mid09 <= now and now <= mid09 + delta:
                            if LTC_hold_flag == 'already_buy':
                                try:
                ...
                print("--------------------------------")
                # while True만 하면 과부하 걸리므로 time.sleep(1)로 1초마다 확인 & import time 도 위에 반드시 써주기
                time.sleep(1)
